...
/**
  @brief        Manage Controller ( 管理員列表控制器 )
**/
...
class Manage extends Controller
{
    ...
    **/
    public function _datalist( )
    {
        ...
        # 取出所有帳號
        # ----------------------------------------------------------------------
        $dataArr =  Bootstart::$_mod[ 'Users_Model' ]->getUsers( 0, 255 );
        # ----------------------------------------------------------------------
        # 防呆 有資料才做
        # ----------------------------------------------------------------------
        if( $dataArr )
    ...
    private function _post( )
    {
        if ( Core_IsPost() && $_POST != '' )
        {
            ...
            else
            {
                ...
                if ( ! $account  )
                {
                    show_errormsg( Bootstart::$_lib[ 'Core_Input' ]->getRuleMsg( 'account' ) );
                }
            }
            # ------------------------------------------------------------------
            # 非必填 檢查有送資料在檢查格式
            ...
            if( !empty( $_POST[ 'password' ] ) )
            {
                ...
                if ( ! $pass  )
                {
                    show_errormsg( Bootstart::$_lib[ 'Core_Input' ]->getRuleMsg( 'pass' ) );
                }
            }
            # ------------------------------------------------------------------
            # 過濾『狀態』
