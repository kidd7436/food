...
class Login extends Controller
{
    ...
    **/
    public function authImage( $_EntryStr )
    {
        ...
        # 使用扭曲版驗證碼
        # ----------------------------------------------------------------------
        $this->Core_Widget->captchaNum( $_EntryStr );
    }
    ...
    public function index()
    {
        ...
        if ( $this->Core_Input->post( 'login' , 'string' ) )
        {
            ...
            if ( $post_authnum == $_EntryStrArr[0] )
            {
                ...
                # 驗證POST過來的帳號
                # --------------------------------------------------------------
                $Post_login = $this->Core_Input->post( "login" , 'mail' );
                ...
                if ( $UserArr['User_Count'] == 1 )
                {
                    ...
                    $_SESSION['Login_user']['phone'] = $UserArr['User_Data']['phone'];
                    $_SESSION['Login_user']['name'] = $UserArr['User_Data']['name'];
                    $_SESSION['Login_user']['address'] = $UserArr['User_Data']['address'];
                    # ----------------------------------------------------------
                    # 先將SESSION資料寫入
                    # ----------------------------------------------------------
                    session_commit();
